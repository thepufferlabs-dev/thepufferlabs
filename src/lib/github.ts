...
/**
 * Extract a short excerpt (first paragraph after the title).
 */
...
/**
 * Fetch all repos that have a docs/ folder, using their README as blog content.
 * Repos without a docs/ folder are skipped.
 */
export async function fetchAllBlogs(): Promise<BlogEntry[]> {
  let repos: GHRepo[];
  try {
    repos = await fetchAllRepos();
  } catch {
  ...
  for (let i = 0; i < repos.length; i += batchSize) {
    ...
    const results = await Promise.all(
      batch.map(async (repo) => {
        try {
          ...
          const title = extractTitle(readme, humanizeName(repo.name));
          const excerpt = extractExcerpt(readme) || repo.description || "";

          // Find the first .md file under docs/ for the "View docs" link
          const docs = discoverDocs(tree);
          const allFiles = docs.flatMap((f) => f.files);
          const firstDoc = allFiles.length > 0 ? allFiles[0].path : "";
          ...
          return {
            ...
            firstDocPath: firstDoc ? firstDoc.replace(/\.md$/, "") : "",
          } satisfies BlogEntry;
        } catch {
          return null;
        }
    ...
    entries.push(...results.filter((e): e is BlogEntry => e !== null));
  }
