...
export const metadata = {
  title: "Docs — ThePufferLabs",
  description:
    "Browse deep technical documentation across all ThePufferLabs projects. Auto-discovered from GitHub.",
};
...
export default async function DocsIndexPage() {
  ...
  const totalDocs = registry.reduce(
    (sum, r) => sum + getAllDocFiles(r).length,
    0
  );
  ...
  return (
    <div className="min-h-screen bg-navy">
      {/* Header */}
      <header className="border-b border-white/5 bg-[#0B1120]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted mt-0.5">
                Auto-discovered from GitHub &middot; Deep technical
                documentation
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <div className="rounded-lg border border-white/5 bg-white/[0.02] px-4 py-3">
              ...
              <p className="text-[11px] text-text-dim font-mono uppercase tracking-wider">
                Projects
              </p>
            </div>
            <div className="rounded-lg border border-white/5 bg-white/[0.02] px-4 py-3">
        ...
        {categoryOrder.map((category) => {
          ...
          return (
            <section key={category} className="mb-14">
              <h2 className="text-xs font-semibold tracking-[0.2em] uppercase text-text-dim mb-5 font-mono">
                {category}
              </h2>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {repos.map((config) => {
                  ...
                  const href = firstFile
                    ? `${basePath}/docs/${config.repo}/${firstFile.path.replace(/\.md$/, "")}/`
                    : `${basePath}/docs/`;
                  const langBreakdown = languagePercentages(
                    config.languageStats
                  );

                  return (
                    <div
                      key={config.repo}
                      className="group rounded-xl border border-white/5 bg-white/[0.02] hover:border-teal/20 hover:bg-white/[0.04] transition-all duration-300 flex flex-col"
                    >
                      {/* Card header */}
                      <div className="p-5 pb-0 flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <Link href={href}>
                            <h3 className="text-base font-semibold text-text-primary group-hover:text-teal transition-colors">
                              {config.name}
                            </h3>
                          </Link>
                          <a
                            ...
                            className="shrink-0 ml-2 text-text-dim hover:text-teal transition-colors"
                        ...
                        {config.topics.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-3">
                            {config.topics.slice(0, 5).map((topic) => (
                              <span
                                key={topic}
                                className="inline-block px-1.5 py-0.5 rounded text-[9px] font-mono bg-teal/5 text-teal/60 border border-teal/10"
                              >
                                {topic}
                              </span>
                            ))}
                            {config.topics.length > 5 && (
                              <span className="text-[9px] text-text-dim/50 self-center">
                                +{config.topics.length - 5}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="mb-3">
                            ...
                            <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                              {langBreakdown.slice(0, 4).map((lang) => (
                                <span
                                  key={lang.language}
                                  className="inline-flex items-center gap-1 text-[9px] text-text-dim/60"
                                >
                                  <span
                      ...
                      {/* Card footer */}
                      <div className="px-5 pb-4 pt-3 border-t border-white/5 mt-auto">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-[10px] text-text-dim font-mono">
                            {files.length}{" "}
                            {files.length === 1 ? "doc" : "docs"}
                          </span>
                          <Link
                            href={href}
                            className="text-xs text-teal/60 hover:text-teal transition-colors font-medium"
                          >
                            Browse &rarr;
                          </Link>
                        </div>
                        ...
                        <ul className="space-y-0.5">
                          {files.slice(0, 3).map((file) => (
                            <li
                              key={file.path}
                              className="flex items-center justify-between text-[11px] text-text-dim"
                            >
                              ...
                              {file.size > 0 && (
                                <span className="text-text-dim/40 ml-2 shrink-0">
                                  {getEstimatedReadTime(file.size)}
                                </span>
                              )}
                            </li>
                          ))}
                          {files.length > 3 && (
                            <li className="text-[11px] text-text-dim/50 italic">
                              +{files.length - 3} more
                            </li>
                          )}
                        </ul>
        ...
        {registry.length === 0 && (
          <div className="text-center py-20">
            <p className="text-text-muted">
              No documentation found. Add a{" "}
              <code className="text-teal">docs/</code> folder with{" "}
              <code className="text-teal">.md</code> files to your repos.
            </p>
          </div>
        )}
      </main>
      ...
      <footer className="border-t border-white/5 py-8">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
          <Link
            href={`${basePath}/`}
            className="text-sm text-text-dim hover:text-teal transition-colors"
          >
            &larr; Back to ThePufferLabs
          </Link>
          <p className="text-xs text-text-dim font-mono">
            &copy; {new Date().getFullYear()} ThePufferLabs &middot; Powered by
            GitHub API
          </p>
        </div>
      </footer>
