...
function buildOption(data: IndicatorTrend[], title: string): echarts.EChartsOption {
  const c = themeColors();
  ...
  const series = countries.map((code) => {
    ...
    return {
      ...
      type: "bar" as const,
      barGap: "10%",
      itemStyle: { color: COUNTRY_COLORS[code] ?? "#888", borderRadius: [3, 3, 0, 0] },
  ...
  return {
    ...
    title: { text: title, left: "center", textStyle: { color: c.text, fontSize: 18, fontWeight: 600 } },
    tooltip: {
      trigger: "axis",
    ...
    legend: { bottom: 10, textStyle: { color: c.textMuted, fontSize: 12 }, icon: "circle" },
    ...
    yAxis: {
      ...
      axisLabel: { color: c.textFaint, fontSize: 11, formatter: (v: number) => `${v}%` },
    },
    series,
  };
}
...
export default function YoYGrowthChart({ data, title }: Props) {
  ...
  useEffect(() => {
    ...
    chart.setOption(buildOption(data, title), true);
